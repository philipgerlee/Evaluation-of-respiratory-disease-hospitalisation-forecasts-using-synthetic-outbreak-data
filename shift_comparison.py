...
if __name__ =='__main__':
    ...
    mobility_n=np.pad(mobility_n,(0,shift), 'edge')

    n_infectious_n = n_infectious[shift:]
    n_infectious_n=np.pad(n_infectious_n,(0,shift), 'edge')
    ...
    for point in indexs_points:
        ...
        for index, model in enumerate(models3D):
                try:
                    data3D=np.array([n_hospitalized, n_infectious_n, mobility_n])
    ...
    with open('./results/thinning_evaluation/evaluation_with_RMSE_on_pandemic_'+str(mob_of_the_pandemic)+'_'+str(number_of_the_pandemic)+'_and_reach_='+str(reach)+'_shift_='+str(shift)+'.json', 'w') as f:
            json.dump(dico_rmse, f)
    print('test')
    with open('./results/thinning_evaluation/evaluation_with_WIS_on_pandemic_'+str(mob_of_the_pandemic)+'_'+str(number_of_the_pandemic)+'_and_reach_='+str(reach)+'_shift_='+str(shift)+'.json', 'w') as f:
