...
if __name__ =='__main__':
    ...
    lam = rng.gamma(shape=noise, scale=n_infectious/np.maximum(noise,1e-12))
    n_infectious_n=rng.poisson(lam).astype(int)
    ...
    for point in indexs_points:
        ...
        for index, model in enumerate(models3D):
                try:
                    data3D=np.array([n_hospitalized, n_infectious_n, mobility])
    ...
    with open('./results/thinning_evaluation/evaluation_with_RMSE_on_pandemic_'+str(mob_of_the_pandemic)+'_'+str(number_of_the_pandemic)+'_and_reach_='+str(reach)+'_noiseinc_='+str(noise)+'.json', 'w') as f:
            json.dump(dico_rmse, f)
    print('test')
    with open('./results/thinning_evaluation/evaluation_with_WIS_on_pandemic_'+str(mob_of_the_pandemic)+'_'+str(number_of_the_pandemic)+'_and_reach_='+str(reach)+'_noiseinc_='+str(noise)+'.json', 'w') as f:
